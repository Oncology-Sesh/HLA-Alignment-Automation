...
# Finally the alignment is then printed as per the IPD-IMGT format. 
...
#################### FUNCTION-1: Querry HLA ##########################################
#' Query HLA Allele Coding Sequence from IPD-IMGT/HLA Database
#'
#' This function queries the IPD-IMGT/HLA database for a specific HLA allele reference ID 
#' and retrieves its coding sequence.
#'
#' @param ID A character string representing the HLA allele reference (e.g., "HLA-A*01:01:01:01").
#'
#' @return A character string containing the coding sequence of the specified HLA allele.
#'
#' @details The function sends a request to the EBI IPD-IMGT/HLA API, fetches the allele data in JSON format,
#' and extracts the coding sequence from the response.
#'
#' @examples
#' \dontrun{
#' coding_seq <- query_HLA("HLA-A*01:01:01:01")
#' print(coding_seq)
#' }
#'
...
get_alt_codon_label_line <- function(block_start,block_end,codon_size,codon_start_num, codon_label_width = 4) {
  ...
  codon_indices <- rep(" ", block_width)# Concat the codon_indices vector 
  cat(codon_indices) ## Integer offset: how many complete codons before this block?
  ...
  for (i in seq(1, block_width, by = codon_size)) {
    ...
    if (codon_num %% 5 == 0 && codon_num != codon_start_num && (i + codon_label_width - 1) <= block_width) {
...
generate_hla_alighnment <- function(allele1, allele2, locus, filepath, reference = NULL){
  ...
  coding2 <- query_HLA(ID2) # Allele-2
  codes = list(coding0,coding1,coding2) # List of all three
